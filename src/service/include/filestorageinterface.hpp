 ...
 * 
 * @details Определяет контракт для реализации адаптеров различных типов хранилищ.
 *          Все методы являются чисто виртуальными, что делает класс абстрактным.
 * 
 * @note Реализует паттерн "Адаптер" для унификации работы с разными типами хранилищ
 */
...
public:
     ...
     * 
     * @note Реализация должна рекурсивно обходить поддиректории, если это поддерживается хранилищем
     */
     ...
     * 
     * @note Должен вызываться перед другими операциями. Может выполнять аутентификацию.
     */
     ...
     * 
     * @note Для событийных хранилищ (локальные/SMB) использует механизмы inotify/ReadDirectoryChanges.
     *       Для протоколов без событий (FTP) реализует периодический опрос.
     */
     ...
     * 
     * @note Коллбэк должен вызываться в потоке мониторинга. Реализации должны гарантировать
     *       потокобезопасность при вызове.
     */
...
protected:
    ...
    FileDetectedCallback onFileDetected_; ///< Коллбэк для уведомлений о новых файлах
};