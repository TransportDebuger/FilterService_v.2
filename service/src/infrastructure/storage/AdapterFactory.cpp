...
AdapterFactory::AdapterFactory() { registerBuiltinAdapters(); }

std::unique_ptr<IFileStorage> AdapterFactory::createAdapter(
    const SourceConfig& config,
    std::shared_ptr<stc::logger::ILogger> logger) const {
  ...
  try {
    ...
    if (logger) {
      logger->Info("Created adapter for type: " + config.type +
                   ", path: " + config.path);
    }
    return adapter;
  } catch (const std::exception& e) {
    if (logger) {
      logger->Error("Failed to create adapter for type " + config.type + ": " +
                    e.what());
    }
