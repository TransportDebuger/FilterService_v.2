...
stc::logger::LogLevel LoggerFactory::ParseLogLevel(
...
std::shared_ptr<stc::logger::IRotationPolicy>
LoggerFactory::CreateRotationPolicy(
  ...
  if (rc.type == "size") {
    return std::make_shared<stc::logger::SizeRotationPolicy>(rc.max_size_bytes,
                                                             rc.max_archives);
  }
  if (rc.type == "time") {
    return std::make_shared<stc::logger::TimeRotationPolicy>(rc.interval_sec,
                                                             rc.time_format);
  }
  if (rc.type == "circular") {
...
std::shared_ptr<stc::logger::ILogSink> LoggerFactory::CreateSink(
  ...
  auto filter =
      std::make_shared<stc::logger::LevelFilter>(ParseLogLevel(config.level));
  auto rotation = CreateRotationPolicy(config.rotation);

  if (config.type == "console") {
    return std::make_shared<stc::logger::ConsoleSink>(std::move(formatter),
                                                      std::move(filter));
  }

  if (config.type == "sync_file") {
    return std::make_shared<stc::logger::FileSink>(
        config.file_path, std::move(formatter), std::move(filter),
        std::move(rotation));
  }
  ...
  throw std::invalid_argument("LoggerFactory: Unsupported sink type: " +
                              config.type);
}
