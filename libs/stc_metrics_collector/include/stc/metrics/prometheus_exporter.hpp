...
@brief Реализует паттерн Visitor для сериализации атомарных метрик в текстовый
формат Prometheus.
*/
class PrometheusExporter final : public IExporterVisitor {
 public:
  ...
  @param[in] out Поток вывода (например, std::ostringstream или буфер
  HTTP-ответа).
  @param[in] namespace_prefix Префикс неймспейса (например, "xmlfilter"). Если
  пуст, префикс не добавляется.
  */
  ...
  */
  void Visit(std::string_view name, std::string_view help,
             const ICounter& counter) override;
  ...
  */
  void Visit(std::string_view name, std::string_view help,
             const IHistogram& histogram) override;
 ...
 private:
  ...
  @return std::string Полное имя в формате "{prefix}_{name}" или "{name}", если
  префикс пуст.
  */
...
*/
[[nodiscard]] std::string ExportToPrometheus(
    const IMetricsRegistry& registry, std::string_view namespace_prefix = "");
