...
class FtpFileAdapter : public IFileStorage {
 public:
   ...
   * @throw std::invalid_argument Если URL некорректен или отсутствуют
  ...
  /// @brief Останавливает мониторинг, разрывает соединение и освобождает
  /// ресурсы libcurl.
  ~FtpFileAdapter() override;

  /**
   * @brief Возвращает список файлов в удаленной директории, соответствующих
   * маске.
   * @param[in] path Относительный путь к директории на FTP-сервере.
   * @return std::vector<std::string> Вектор имен найденных файлов.
   * @throw std::runtime_error При ошибках инициализации CURL, сетевого
   * взаимодействия или авторизации.
   */
   ...
   * @param[in] localPath Абсолютный или относительный путь для сохранения на
   * локальном диске.
   * @throw std::invalid_argument При некорректных путях (базовая проверка
   * validatePath).
   * @throw std::ios_base::failure При ошибках создания локального файла, записи
   * на диск или разрыва соединения.
   */
   ...
   * @throw std::invalid_argument При некорректных путях или отсутствии
   * локального файла.
   * @throw std::runtime_error При ошибках получения размера файла (stat) или
  ...
  /**
   * @brief Проверяет доступность FTP-сервера и устанавливает базовое
   * соединение.
   * @throw std::runtime_error Если сервер недоступен или неверны учетные
   * данные.
   */
   ...
   * @param[in] callback Функция, которая будет вызвана при обнаружении файла,
   * соответствующего маске конфигурации источника.
   */
 ...
 private:
  ...
  /// @private Проверяет доступность FTP-сервера посредством тестового
  /// LIST-запроса.
