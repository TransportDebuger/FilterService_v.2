...
class IDirectoryMonitor {
 public:
  ...
  /// @brief Сигнатура callback-функции для обработки событий файловой системы.
  ...
  /**
    ...
    @throw std::runtime_error Любая реализация обязана выбросить это исключение
    при невозможности инициализации системных дескрипторов или исчерпании
    ресурсов.
    @throw std::system_error Любая реализация обязана выбросить это исключение
    при ошибках базовых системных вызовов.
    */
