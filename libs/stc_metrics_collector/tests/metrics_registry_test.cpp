...
@brief Unit-тесты для ядра библиотеки stc::metrics (реестр, атомарные примитивы, NoOp).
*/
...
TEST(MetricsRegistryTest, ThrowsOnDuplicateRegistration) {
    ...
    EXPECT_THROW(
        registry.RegisterCounter("metric_a", "Duplicate"), 
...
TEST(MetricsRegistryTest, ThrowsOnEmptyMetricName) {
    ...
    EXPECT_THROW(
        registry.RegisterCounter("", "Empty name"), 
        std::invalid_argument
    );
}
...
TEST(MetricsRegistryTest, HistogramValidation) {
    ...
    EXPECT_THROW(
        registry.RegisterHistogram("hist_empty", "Help", {}), 
        std::invalid_argument
    );
...
TEST(ConcurrencyTest, NoDataRaceOnHistogram) {
    ...
    constexpr uint64_t kExpectedCount = kThreads * kIterations * 4; // 4 observe per iteration
    constexpr double kExpectedSum = kThreads * kIterations * (0.5 + 3.0 + 7.0 + 15.0);
