...
std::vector<std::string> LocalStorageAdapter::listFiles(
  ...
  try {
    if (!std::filesystem::exists(path) ||
      ...
      if (logger_)
        logger_->Warning("Directory does not exist or is not accessible: " +
                         path);
      return files;
    }
    ...
    if (logger_)
      logger_->Debug("Found " + std::to_string(files.size()) +
                     " files in: " + path);
  } catch (const std::filesystem::filesystem_error& e) {
    if (logger_)
      logger_->Error("Filesystem error in listFiles: " + std::string(e.what()));
...
void LocalStorageAdapter::downloadFile(const std::string& remotePath,
  ...
  try {
    ...
    if (logger_)
      logger_->Info("File copied from " + remotePath + " to " + localPath);
  } catch (const std::filesystem::filesystem_error& e) {
    if (logger_)
      logger_->Error("Failed to copy file: " + std::string(e.what()));
...
void LocalStorageAdapter::upload(const std::string& localPath,
  ...
  try {
    ...
    std::filesystem::path remoteDir =
        std::filesystem::path(remotePath).parent_path();
    if (!remoteDir.empty()) std::filesystem::create_directories(remoteDir);
    std::filesystem::copy_file(
        localPath, remotePath,
        std::filesystem::copy_options::overwrite_existing);
    if (logger_)
      logger_->Info("File uploaded from " + localPath + " to " + remotePath);
  } catch (const std::filesystem::filesystem_error& e) {
    if (logger_)
      logger_->Error("Failed to upload file: " + std::string(e.what()));
...
void LocalStorageAdapter::startMonitoring() {
  ...
  try {
    ...
    auto callback = [this](stc::fs::IDirectoryMonitor::Event event,
                           const std::string& filePath) {
      handleFileEvent(event, filePath);
    };

    monitor_ = stc::fs::DirectoryMonitor::CreateWithStrategy(
        strategy, config_.path, callback, config_.check_interval);

    monitor_->Start();
    monitoring_ = true;
    if (logger_)
      logger_->Info("Started monitoring [" + config_.monitoring_strategy +
                    "]: " + config_.path);
  } catch (const std::exception& e) {
    if (logger_)
      logger_->Error("Failed to start monitoring: " + std::string(e.what()));
...
void LocalStorageAdapter::handleFileEvent(
    stc::fs::IDirectoryMonitor::Event event, const std::string& filePath) {
  try {
    ...
    if (logger_)
      logger_->Error("Error handling file event: " + std::string(e.what()));
  }
}
...
bool LocalStorageAdapter::matchesFileMask(const std::string& filename) const {
  try {
    ...
    if (logger_)
      logger_->Warning("Invalid file mask regex: '" + config_.file_mask +
                       "', error: " + e.what());
    return true;
  }
}

stc::fs::DirectoryMonitor::MonitoringStrategy
LocalStorageAdapter::resolveStrategy() const {
