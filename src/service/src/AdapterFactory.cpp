...
std::unique_ptr<FileStorageInterface> AdapterFactory::createAdapter(const SourceConfig& config) {
    ...
    try {
        ...
        stc::CompositeLogger::instance().error(
            "Failed to create adapter for type " + config.type + ": " + std::string(e.what())
        );
...
void AdapterFactory::registerBuiltinAdapters() {
    ...
    registerAdapter("local", [](const SourceConfig& config) -> std::unique_ptr<FileStorageInterface> {
        return std::make_unique<LocalStorageAdapter>(config);
    });
    
    // Регистрация SmbFileAdapter
    registerAdapter("smb", [](const SourceConfig& config) -> std::unique_ptr<FileStorageInterface> {
        // Валидация обязательных параметров для SMB
        std::vector<std::string> required_fields = {"username"};
        AdapterFactory::instance().validateRequiredFields(config, required_fields);
        
        return std::make_unique<SmbFileAdapter>(config);
    });
    
    // Регистрация FtpFileAdapter
    registerAdapter("ftp", [](const SourceConfig& config) -> std::unique_ptr<FileStorageInterface> {
