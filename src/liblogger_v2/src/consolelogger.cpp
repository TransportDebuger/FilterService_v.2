...
bool stc::ConsoleLogger::shouldSkipLog(LogLevel level) const { 
    return static_cast<int>(level) < static_cast<int>(currentLevel_.load(std::memory_order_acquire));
}
...
void stc::ConsoleLogger::setConsoleColor(LogLevel level) {
        ...
        switch(level) {
            case LogLevel::LOG_DEBUG:   color = FOREGROUND_BLUE | FOREGROUND_GREEN; break;  // Cyan
            case LogLevel::LOG_INFO:    color = FOREGROUND_GREEN; break;                     // Green
            case LogLevel::LOG_WARNING: color = FOREGROUND_RED | FOREGROUND_GREEN; break;    // Yellow
            case LogLevel::LOG_ERROR:   color = FOREGROUND_RED | FOREGROUND_INTENSITY; break;  // Bright Red
            case LogLevel::LOG_CRITICAL:   color = BACKGROUND_RED | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE; break; // White on Red
            default:               color = FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE; // White
        }
        ...
        switch(level) {
            case LogLevel::LOG_DEBUG:   colorCode = "\033[36m"; break;    // Cyan
            case LogLevel::LOG_INFO:    colorCode = "\033[32m"; break;    // Green
            case LogLevel::LOG_WARNING: colorCode = "\033[33m"; break;    // Yellow
            case LogLevel::LOG_ERROR:   colorCode = "\033[31m"; break;    // Red
            case LogLevel::LOG_CRITICAL:   colorCode = "\033[41m\033[37m"; break; // White on Red
            default:                colorCode = ANSI_COLOR_RESET;
        }
