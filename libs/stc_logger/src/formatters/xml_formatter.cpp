...
void XmlFormatter::AppendTimeIso8601(
  ...
  output.push_back('.');
  if (ms.count() < 10) output.append("00");
  else if (ms.count() < 100) output.push_back('0');
  output.append(std::to_string(ms.count()));
}

void XmlFormatter::AppendEscapedString(std::string& output, std::string_view str) {
  std::size_t start = 0;
  for (std::size_t i = 0; i < str.size(); ++i) {
    const char c = str[i];
    // XML требует экранирования: & < > " ' и управляющих символов (кроме \t \n \r)
    bool needs_escape = (c == '&' || c == '<' || c == '>' || c == '"' || c == '\'');
