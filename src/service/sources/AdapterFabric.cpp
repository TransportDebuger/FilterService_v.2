...
#include "../includes/LocalFileAdapter.hpp"
#include <memory>
#include "AdapterFabric.hpp"
...
std::unique_ptr<FileStorageInterface> AdapterFabric::createAdapter(
    ...
    const std::string& password) {
        switch (type) {
            case SourceType::LOCAL: return std::make_unique<LocalFileAdapter>(path);
            //case SourceType::FTP:   return std::make_unique<FtpFileAdapter>(path, username, password);
            //case SourceType::SFTP:  return std::make_unique<SftpFileAdapter>(path, username, password);
            //case SourceType::SMB:   return std::make_unique<SmbFileAdapter>(path, username, password);
