...
AtomicHistogram::AtomicHistogram(std::vector<double> boundaries) 
    : boundaries_(std::move(boundaries)), bucket_count_(boundaries_.size() + 1) {
    ...
    for (std::size_t i = 1; i < boundaries_.size(); ++i) {
        if (boundaries_[i] <= boundaries_[i - 1]) {
            throw std::invalid_argument("Histogram boundaries must be strictly sorted in ascending order");
        }
    }
...
void MetricsRegistry::RegisterMetric(std::string_view name, std::string_view help, std::shared_ptr<IMetric> metric) {
    if (name.empty()) {
...
std::shared_ptr<ICounter> MetricsRegistry::RegisterCounter(std::string_view name, std::string_view help) {
    auto counter = std::make_shared<AtomicCounter>();
    RegisterMetric(name, help, counter);
    return counter;
}

std::shared_ptr<IGauge> MetricsRegistry::RegisterGauge(std::string_view name, std::string_view help) {
    auto gauge = std::make_shared<AtomicGauge>();
    RegisterMetric(name, help, gauge);
    return gauge;
}

std::shared_ptr<IHistogram> MetricsRegistry::RegisterHistogram(std::string_view name, std::string_view help, std::vector<double> buckets) {
...
void MetricsRegistry::AcceptVisitor(IExporterVisitor& visitor) const {
    ...
    // Используем dynamic_cast для разделения метаданных (name, help) и атомарных данных.
    // Это избавляет от необходимости дублировать строки внутри каждой атомарной метрики.
    for (const auto& record : records_) {
        if (auto counter = dynamic_cast<const ICounter*>(record.instance.get())) {
            visitor.Visit(record.name, record.help, *counter);
        } else if (auto gauge = dynamic_cast<const IGauge*>(record.instance.get())) {
            visitor.Visit(record.name, record.help, *gauge);
        } else if (auto histogram = dynamic_cast<const IHistogram*>(record.instance.get())) {
