...
/**
@class LoggerFactory
@brief Предоставляет статические методы для сборки экземпляра
stc::logger::ILogger.
*/
class LoggerFactory {
 public:
 ...
 private:
  ...
  @param[in] level_str Строка уровня (trace, debug, info, warning, error,
  critical).
  ...
  */
  ...
  @return std::shared_ptr<stc::logger::IRotationPolicy> Указатель на политику
  или nullptr.
  */
