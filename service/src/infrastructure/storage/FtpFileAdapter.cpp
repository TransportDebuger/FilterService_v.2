...
void FtpFileAdapter::downloadFile(const std::string &remotePath,
  ...
  try {
    ...
    if (logger_)
      logger_->Info("FTP file downloaded from " + url + " to " + localPath);
  } catch (...) {
    curl_easy_cleanup(curl);
    throw;
  }
...
void FtpFileAdapter::upload(const std::string &localPath,
  ...
  try {
    ...
    if (logger_)
      logger_->Info("File uploaded from " + localPath + " to FTP: " + url);
  } catch (...) {
    curl_easy_cleanup(curl);
    throw;
  }
...
void FtpFileAdapter::connect() {
  ...
  try {
    ...
    if (logger_)
      logger_->Error("FTP connection failed: " + std::string(e.what()));
...
void FtpFileAdapter::startMonitoring() {
  ...
  try {
    ...
    if (logger_)
      logger_->Info("Started FTP monitoring with " +
                    std::to_string(pollingInterval_.count()) + "s interval");
  } catch (const std::exception &e) {
    monitoring_ = false;
    if (logger_)
      logger_->Error("Failed to start FTP monitoring: " +
                     std::string(e.what()));
...
void FtpFileAdapter::monitoringLoop() {
  while (monitoring_) {
    try {
      ...
      if (logger_)
        logger_->Error("FTP monitoring error: " + std::string(e.what()));
    }
...
bool FtpFileAdapter::matchesFileMask(const std::string &filename) const {
  try {
    ...
    if (logger_)
      logger_->Warning("Invalid FTP file mask regex: " + config_.file_mask);
    return true;
  }
}
