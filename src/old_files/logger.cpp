 ...
 * @date March 2025
 * @brief Реализация статического класса Logger и его вспомогательных
 * компонентов.
 *
 * @details Этот файл содержит реализацию методов для:
 *          - Записи логов в файл и консоль с поддержкой уровней (DEBUG, INFO,
 * WARNING, ERROR, FATAL).
 *          - Ротации лог-файлов при достижении заданного размера.
 *          - Управления fallback-буфером для сохранения логов при ошибках
 * файловой системы.
 ...
 *       - Обработка ошибок файловых операций (открытие/закрытие/переименование)
 * интегрирована в методы.
 *
 * @warning Внимание:
 *          - Методы класса не должны вызываться до инициализации
 * (Logger::init()).
 *          - Рекурсивный вызов методов Logger из одного потока может привести к
 * deadlock из-за использования нерекурсивных мьютексов.
 *
 * @section Потокобезопасность
 * @par Реализация:
 * - Каждый публичный метод защищён мьютексом @b mutex_ для обеспечения
 * атомарности операций.
 * - Fallback-буфер и связанные с ним флаги защищены отдельным мьютексом @b
 * fallbackMutex_.
 * - Критические секции минимальны по времени выполнения для снижения
 * contention.
 *
 * @par Примеры:
 * @code
...
FallbackBufferGuard Logger::fallbackGuard_(Logger::fallbackBuffer_,
...
std::queue<Logger::LogMessage> Logger::logQueue_;
std::mutex Logger::queueMutex_;
std::condition_variable Logger::queueCV_;
std::atomic<bool> Logger::asyncEnabled_(false);
std::thread Logger::workerThread_;
std::atomic<bool> Logger::workerRunning_(false);

void Logger::init(bool async = true, const std::string& path = "") {
  ...
  if (async) {
    asyncEnabled_ = true;
    workerRunning_ = true;
    workerThread_ = std::thread(&Logger::worker);
  }
}
...
void Logger::rotateLog() {
  if (!logFile_.is_open()) {
    std::cerr << COLOR_ERROR << "[ERROR] Log file is not open, cannot rotate."
  ...
  if (rename(filename_.c_str(), oldLog.c_str()) != 0) {
    ...
    try {
      ...
      std::cerr << "Failed to reopen log file after failed rotation: "
                << e.what() << std::endl;
  ...
  try {
    ...
    if (logFile_.is_open()) {
      logFile_ << "[" << getCurrentTime(false)
               << "] [ERROR] Error reopening log file after rotation: "
               << e.what() << std::endl;
      ...
      std::lock_guard<std::mutex> fallbackLock(fallbackMutex_);
      fallbackBuffer_ << "[" << getCurrentTime(false)
                      << "] [ERROR] Error reopening log file after rotation: "
                      << e.what() << std::endl;
      fallbackUsed_ = true;
      std::cerr << "Error reopening log file after rotation: " << e.what()
                << std::endl;
    }
    // Продолжаем логирование в старом файле
  }
  ...
  if (logFile_.is_open()) {
    ...
    if (needsRotation()) {
      try {
        ...
        if (logFile_.is_open()) {
          logFile_ << "[" << getCurrentTime(false)
                   << "] [ERROR] Failed to rotate log file: " << e.what()
                   << std::endl;
          logFile_.flush();  // Принудительно сбрасываем буфер
          std::cerr << COLOR_ERROR
                    << "[ERROR] Failed to rotate log file: " << e.what()
          ...
          fallbackBuffer_ << "[" << getCurrentTime(false)
                          ...
                          << std::endl;
          fallbackUsed_ = true;
          std::cerr << color << "Failed to rotate log file: " << e.what()
                    << COLOR_RESET << std::endl;
        }
        // Продолжаем логирование в старом файле
      }
    ...
    try {
      logFile_ << "[" << getCurrentTime(false) << "] [" << levelStr << "] "
               << message << std::endl;
      logFile_.flush();  // Принудительно сбрасываем буфер
    } catch (const std::exception& e) {
      std::cerr << color << "Error writing to log file: " << e.what()
                << COLOR_RESET << std::endl;
      // Переход на fallback
      std::lock_guard<std::mutex> fallbackLock(fallbackMutex_);
      fallbackBuffer_ << "[" << getCurrentTime(false) << "] [" << levelStr
                      << "] " << message << std::endl;
      fallbackUsed_ = true;
    }

    // Вывод в консоль с цветом
    std::cerr << color << "[" << getCurrentTime(false) << "] [" << levelStr
...
void Logger::setLogSize(size_t logSize) { maxSize_ = logSize; }
...
Logger::LogLevel Logger::strToLogLevel(const std::string& level) {
  ...
  if (lowLevel == "info") {
    ...
    throw std::runtime_error(
        "Fatal error: Unknown type of log level. Run service with --help to "
        "get usage hints.");
  }
