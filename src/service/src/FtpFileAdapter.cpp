...
FtpFileAdapter::FtpFileAdapter(const SourceConfig& config) 
    ...
    stc::CompositeLogger::instance().info(
        "FtpFileAdapter created for: " + ftpUrl_
    );
}
...
void FtpFileAdapter::downloadFile(const std::string& remotePath, 
    ...
    try {
        ...
        stc::CompositeLogger::instance().info(
            "FTP file downloaded from " + url + " to " + localPath
        );
...
void FtpFileAdapter::upload(const std::string& localPath,
    ...
    try {
        ...
        stc::CompositeLogger::instance().info(
            "File uploaded from " + localPath + " to FTP: " + url
        );
...
void FtpFileAdapter::connect() {
    ...
    try {
        ...
        stc::CompositeLogger::instance().info(
            "Connected to FTP server: " + ftpUrl_
        );
        
    } catch (const std::exception& e) {
        stc::CompositeLogger::instance().error(
            "FTP connection failed: " + std::string(e.what())
        );
...
bool FtpFileAdapter::isConnected() const noexcept {
...
void FtpFileAdapter::startMonitoring() {
    ...
    try {
        ...
        stc::CompositeLogger::instance().info(
            "Started FTP monitoring with " + std::to_string(pollingInterval_.count()) + 
            "s interval"
        );
        ...
        monitoring_ = false;
        stc::CompositeLogger::instance().error(
            "Failed to start FTP monitoring: " + std::string(e.what())
        );
...
void FtpFileAdapter::monitoringLoop() {
    while (monitoring_) {
        try {
            ...
            stc::CompositeLogger::instance().error(
                "FTP monitoring error: " + std::string(e.what())
            );
        }
...
bool FtpFileAdapter::matchesFileMask(const std::string& filename) const {
    try {
        ...
        stc::CompositeLogger::instance().warning(
            "Invalid FTP file mask regex: " + config_.file_mask
        );
        return true;
    }
}
