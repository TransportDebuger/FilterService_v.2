...
void LocalStorageAdapter::downloadFile(const std::string& remotePath, const std::string& localPath) {
    ...
    try {
        ...
        std::filesystem::path localDir = std::filesystem::path(localPath).parent_path();
        if (!localDir.empty()) std::filesystem::create_directories(localDir);
        std::filesystem::copy_file(remotePath, localPath, std::filesystem::copy_options::overwrite_existing);
...
void LocalStorageAdapter::upload(const std::string& localPath, const std::string& remotePath) {
    ...
    try {
        ...
        std::filesystem::copy_file(localPath, remotePath, std::filesystem::copy_options::overwrite_existing);
