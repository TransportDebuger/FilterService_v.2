...
void Daemonizer::daemonize() {
  ...
  if (pid < 0) {
    throw std::runtime_error(
        "Daemonizer: Failed to fork (errno=" + std::to_string(errno) + ")");
  }
  if (pid > 0) {
  ...
  if (setsid() < 0) {
    throw std::runtime_error(
        "Daemonizer: Failed to create new session (setsid)");
  }
  ...
  pid = fork();
  if (pid < 0) {
    throw std::runtime_error("Daemonizer: Failed to fork second time (errno=" +
                             std::to_string(errno) + ")");
  }
  if (pid > 0) {
    exit(EXIT_SUCCESS);
