...
PrometheusExporter::PrometheusExporter(std::ostream& out, std::string_view namespace_prefix)
...
void PrometheusExporter::Visit(std::string_view name, std::string_view help, const ICounter& counter) {
    ...
    if (full_name.size() < 6 || full_name.substr(full_name.size() - 6) != "_total") {
        full_name += "_total";
    }
...
void PrometheusExporter::Visit(std::string_view name, std::string_view help, const IGauge& gauge) {
...
void PrometheusExporter::Visit(std::string_view name, std::string_view help, const IHistogram& histogram) {
    ...
    for (std::size_t i = 0; i < boundary_count; ++i) {
        ...
        out_ << full_name << "_bucket{le=\"" << boundaries[i] << "\"} " << cumulative << "\n";
    }
...
std::string ExportToPrometheus(const IMetricsRegistry& registry, std::string_view namespace_prefix) {
