...
/**
@class AtomicGauge
@brief Потокобезопасная lock-free реализация датчика (значения, которое может меняться в обе стороны).
*/
...
@brief Потокобезопасная lock-free реализация гистограммы с предопределенными бакетами.
*/
...
public:
    ...
    */
    ...
    @param[in] value Наблюдаемое значение. Валидация (например, проверка на NaN или Inf) 
                     намеренно не выполняется на «горячем пути» ради максимальной производительности. 
                     Ответственность за фильтрацию некорректных данных лежит на вызывающем коде.
    */
...
private:
    ...
    /// @private Массив атомарных счетчиков для каждого бакета (размер = boundaries_.size() + 1).
...
@brief Конкретная реализация реестра метрик. Обеспечивает фазу регистрации и lock-free сбор.
*/
...
public:
    ...
    */
    std::shared_ptr<ICounter> RegisterCounter(std::string_view name, std::string_view help) override;
    ...
    @throw std::invalid_argument Если имя пустое или уже зарегистрировано.
    */
    std::shared_ptr<IGauge> RegisterGauge(std::string_view name, std::string_view help) override;
    ...
    @throw std::invalid_argument Если имя пустое, уже зарегистрировано, или массив бакетов невалиден.
    */
    std::shared_ptr<IHistogram> RegisterHistogram(std::string_view name, std::string_view help, std::vector<double> buckets) override;
...
private:
    ...
    @private
    @struct MetricRecord
    @brief Внутренняя структура для связывания метаданных (имя, описание) с экземпляром метрики.
    Разделение необходимо для того, чтобы атомарные примитивы не хранили строковые данные, 
    избегая аллокаций и накладных расходов на «горячем пути» (hot path).
    */
    struct MetricRecord {
        /// @private 
        std::string name; ///< Уникальное имя метрики, передаваемое в экспортер при сериализации.
        
        /// @private
        std::string help; ///< Описание метрики (директива HELP), передаваемое в экспортер.
        
        /// @private
        std::shared_ptr<IMetric> instance; ///< Умный указатель на экземпляр метрики, гарантирующий корректный жизненный цикл и потокобезопасный доступ.
    };
    ...
    @throw std::invalid_argument Если имя пустое или уже зарегистрировано.
    */
    void RegisterMetric(std::string_view name, std::string_view help, std::shared_ptr<IMetric> metric);

    /// @private Хранилище зарегистрированных метрик (порядок важен для стабильного экспорта).
