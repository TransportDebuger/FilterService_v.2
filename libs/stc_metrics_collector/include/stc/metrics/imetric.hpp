...
/// Опережающее объявление посетителя для разрыва циклических зависимостей заголовков.
class IExporterVisitor;

/**
@class IMetric
@brief Базовый полиморфный интерфейс для любой метрики. Позволяет реестру хранить разнородные метрики в едином контейнере.
*/
class IMetric {
public:
    /// @brief Виртуальный деструктор для безопасного удаления производных классов через базовый указатель.
...
public:
    ...
    */
    virtual void Visit(std::string_view name, std::string_view help, const ICounter& counter) = 0;
    ...
    */
    virtual void Visit(std::string_view name, std::string_view help, const IGauge& gauge) = 0;
    ...
    */
    virtual void Visit(std::string_view name, std::string_view help, const IHistogram& histogram) = 0;
};
