 ...
 * @brief Адаптер для работы с локальной файловой системой и точками
 * монтирования SMB.
 * @version 2.2.0
 * @date 2026-07-24
 */
...
class LocalStorageAdapter : public IFileStorage {
 public:
  ...
  /// @brief Виртуальный деструктор. Останавливает мониторинг и освобождает
  /// ресурсы.
   ...
   * @throw std::ios_base::failure При ошибках копирования или создания
   * директорий.
   */
  ...
  /**
   * @brief Проверяет существование директории и рекурсивно создает её при
   * необходимости.
   ...
   * @throw std::runtime_error Если соединение не установлено, мониторинг уже
   * запущен или стратегия не поддерживается.
   */
 ...
 private:
   ...
   * @throw std::runtime_error Если создание директории невозможно или путь
   * указывает на файл.
   */
   ...
   */
  void handleFileEvent(stc::fs::IDirectoryMonitor::Event event,
  ...
  /**
   * @private
   * @brief Транслирует строковую стратегию мониторинга из конфигурации в
   * перечисление.
