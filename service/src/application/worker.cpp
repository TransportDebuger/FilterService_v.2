...
#include "../infrastructure/storage/AdapterFactory.hpp"
#include "../infrastructure/system/signal_mask_guard.hpp"
...
Worker::Worker(const SourceConfig &config,
  ...
  try {
    ...
    adapter_ = factory.createAdapter(config_, logger_);
    ...
    if (logger_)
      logger_->Info("Worker created for source: " + config_.name +
                    " (type: " + config_.type + "), " + workerTag_);
  } catch (const std::exception &e) {
    if (logger_)
      logger_->Error("Failed to create worker: " + std::string(e.what()) +
                     ", " + workerTag_);
    throw;
  }
}
...
Worker::~Worker() {
  try {
    if (logger_) logger_->Debug("Worker destructor called for " + workerTag_);
    stopGracefully();
    if (logger_) logger_->Debug("Worker destroyed, " + workerTag_);
  } catch (...) {
...
void Worker::start() {
  ...
  if (running_) {
    if (logger_) logger_->Warning("Worker already running, " + workerTag_);
    return;
  }
  try {
    validatePaths();
    ...
    if (logger_)
      logger_->Info("Worker started monitoring: " + config_.path + ", " +
                    workerTag_);
  } catch (const std::exception &e) {
    running_ = false;
    if (logger_)
      logger_->Error("Failed to start worker: " + std::string(e.what()) + ", " +
                     workerTag_);
    throw;
  }
}
...
void Worker::stop() {
  {
    ...
    running_ = false;
    paused_ = false;
  }  // КРИТИЧНО: Освобождаем мьютекс перед join, чтобы Worker::run() мог
     // завершиться
  ...
  if (logger_) logger_->Info("Worker stopped, " + workerTag_);
}
...
void Worker::pause() {
  ...
  paused_ = true;
  if (logger_) logger_->Info("Worker paused, " + workerTag_);
}
...
void Worker::resume() {
  ...
  paused_ = false;
  cv_.notify_all();
  if (logger_) logger_->Info("Worker resumed, " + workerTag_);
}

void Worker::restart() {
  if (logger_) logger_->Info("Restarting worker, " + workerTag_);
  stop();
...
void Worker::stopGracefully() {
  if (logger_) logger_->Debug("Worker::stopGracefully() ENTER " + workerTag_);
  if (!running_) return;
  while (processing_) {
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
  }
  stop();
  if (logger_) logger_->Debug("Worker::stopGracefully() EXIT " + workerTag_);
}
...
const SourceConfig &Worker::getConfig() const noexcept { return config_; }
...
void Worker::run() {
  try {
    while (running_) {
      ...
      cv_.wait_for(lock, std::chrono::seconds(config_.check_interval), [this] {
      ...
      if (std::chrono::steady_clock::now() - start_time_ >
        ...
        if (logger_)
          logger_->Debug("Worker stats - Avg duration: " +
                         std::to_string(avg_duration_.load()) + "s, " +
                         workerTag_);
      }
    }
  } catch (const std::exception &e) {
    if (logger_)
      logger_->Error("Worker crashed: " + std::string(e.what()) + ", " +
                     workerTag_);
    running_ = false;
  }
}
...
void Worker::processFile(const std::string &filePath) {
    ...
    try {
        ...
        if (config_.filtering_enabled) {
            XMLProcessor processor(config_, logger_, filter_list_manager_);
            result = processor.process(filePath);

            if (!result.success) {
                if (source_metrics_.files_failed)
                    source_metrics_.files_failed->Increment(1.0);
                if (global_metrics_.files_failed)
                    global_metrics_.files_failed->Increment(1.0);

                if (result.error_type == ProcessingResult::ErrorType::WRITE) {
                    if (source_metrics_.files_failed_write)
                        source_metrics_.files_failed_write->Increment(1.0);
                } else {
                    if (source_metrics_.files_failed_parse)
                        source_metrics_.files_failed_parse->Increment(1.0);
                }

                handleFileError(filePath, "XML processing failed");
                processing_ = false;
                if (source_metrics_.worker_state)
                    source_metrics_.worker_state->Set(0.0);
                return;
            }

            if (result.bypass) {
                // Режим bypass: перемещаем исходный файл в processed без изменений
                std::string dst = (fs::path(config_.processed_dir) / filename).string();
                moveToProcessed(filePath, dst);

                // Метрики для режима bypass
                if (source_metrics_.files_processed)
                    source_metrics_.files_processed->Increment(1.0);
                if (global_metrics_.files_processed)
                    global_metrics_.files_processed->Increment(1.0);
                if (source_metrics_.bytes_processed)
                    source_metrics_.bytes_processed->Increment(
                        static_cast<double>(result.bytes_processed));
                if (global_metrics_.bytes_processed)
                    global_metrics_.bytes_processed->Increment(
                        static_cast<double>(result.bytes_processed));
            } else {
                // Режим фильтрации: результат уже записан в processed и, возможно, в excluded.
                // Исходный файл удаляется, чтобы не перезаписывать результат.
                std::error_code ec;
                fs::remove(filePath, ec);
                if (ec && logger_) {
                    logger_->Warning("Failed to remove source file after filtering: " +
                                     filePath + ", error: " + ec.message() + ", " + workerTag_);
                }

                // Метрики для режима фильтрации
                if (source_metrics_.files_processed)
                    source_metrics_.files_processed->Increment(1.0);
                if (global_metrics_.files_processed)
                    global_metrics_.files_processed->Increment(1.0);

                if (source_metrics_.records_processed)
                    source_metrics_.records_processed->Increment(
                        static_cast<double>(result.records_processed));
                if (global_metrics_.records_processed)
                    global_metrics_.records_processed->Increment(
                        static_cast<double>(result.records_processed));

                if (source_metrics_.records_matched)
                    source_metrics_.records_matched->Increment(
                        static_cast<double>(result.records_matched));
                if (global_metrics_.records_matched)
                    global_metrics_.records_matched->Increment(
                        static_cast<double>(result.records_matched));
            ...
            // Режим без фильтрации: исходный файл перемещается в processed
            std::string dst = (fs::path(config_.processed_dir) / filename).string();
            moveToProcessed(filePath, dst);
            ...
            result.success = true;
            try {
                result.bytes_processed = fs::file_size(filePath);
            } catch (...) {
        ...
        if (source_metrics_.duration_hist)
            source_metrics_.duration_hist->Observe(duration);
        if (global_metrics_.duration_hist)
            global_metrics_.duration_hist->Observe(duration);
        ...
        if (logger_)
            logger_->Info("Successfully processed file: " + filePath +
                          " (hash: " + fileHash.substr(0, 8) + "...) in " +
                          std::to_string(duration) + "s, " + workerTag_);
...
void Worker::moveToProcessed(const std::string &filePath,
  ...
  try {
    ...
    if (!fs::exists(dir)) {
      ...
      if (logger_) logger_->Info("Created directory: " + dir.string());
    }
    ...
    if (logger_)
      logger_->Debug("Moved file from " + filePath + " to " + processedPath +
                     ", " + workerTag_);
  } catch (const fs::filesystem_error &e) {
...
void Worker::handleFileError(const std::string &filePath,
  ...
  try {
    if (!config_.bad_dir.empty()) {
      ...
      if (logger_)
        logger_->Warning("Moved failed file to bad directory: " + badPath +
                         ", " + workerTag_);
    }
    if (logger_)
      logger_->Error("Failed to process file " + filePath + ": " + error +
                     ", " + workerTag_);
  } catch (const std::exception &e) {
    if (logger_)
      logger_->Error("Failed to handle file error: " + std::string(e.what()) +
                     ", " + workerTag_);
  }
}
...
void Worker::restartMonitoring() {
  ...
  if (!adapter_) {
    if (logger_)
      logger_->Warning("Worker " + config_.name +
                       " has no adapter to restart, " + workerTag_);
    return;
  }
  try {
    if (running_) {
      adapter_->stopMonitoring();
      if (logger_)
        logger_->Debug("Worker " + config_.name + " monitoring stopped, " +
                       workerTag_);
      adapter_->startMonitoring();
      if (logger_)
        logger_->Info("Worker " + config_.name + " monitoring restarted, " +
                      workerTag_);
    } else {
      if (logger_)
        logger_->Warning("Worker " + config_.name +
                         " is not running, cannot restart monitoring, " +
                         workerTag_);
    }
  } catch (const std::exception &e) {
    if (logger_)
      logger_->Error("Failed to restart monitoring for worker " + config_.name +
                     ": " + std::string(e.what()) + ", " + workerTag_);
    throw;
  }
}
