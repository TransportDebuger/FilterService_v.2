...
TEST(PrometheusExporterTest, CounterFormattingAndTotalSuffix) {
  ...
  EXPECT_TRUE(
      Contains(output, "# HELP my_prefix_requests_total Total requests\n"));
...
TEST(PrometheusExporterTest, GaugeFormatting) {
  ...
  EXPECT_TRUE(Contains(
      output, "# HELP my_prefix_active_connections Current connections\n"));
...
TEST(PrometheusExporterTest, HistogramFormattingAndCumulativeBuckets) {
  ...
  auto hist = registry.RegisterHistogram("duration_seconds", "Time spent",
                                         {1.0, 5.0, 10.0});
  ...
  EXPECT_TRUE(
      Contains(output, "# HELP my_prefix_duration_seconds Time spent\n"));
  EXPECT_TRUE(
      Contains(output, "# TYPE my_prefix_duration_seconds histogram\n"));

  EXPECT_TRUE(
      Contains(output, "my_prefix_duration_seconds_bucket{le=\"1\"} 2\n"));
  EXPECT_TRUE(
      Contains(output, "my_prefix_duration_seconds_bucket{le=\"5\"} 4\n"));
  EXPECT_TRUE(
      Contains(output, "my_prefix_duration_seconds_bucket{le=\"10\"} 5\n"));
  EXPECT_TRUE(
      Contains(output, "my_prefix_duration_seconds_bucket{le=\"+Inf\"} 7\n"));
...
TEST(PrometheusExporterTest, NoOpRegistryExportIsEmpty) {
  ...
  // NoOp метрики игнорируются экспортером (dynamic_cast к Atomic* вернет
  // nullptr). Следовательно, вывод должен быть полностью пустым, без мусора.
