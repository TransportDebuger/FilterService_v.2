...
public:
    ...
    using CreatorFunction = std::function<std::unique_ptr<FileStorageInterface>(const SourceConfig&)>;
     ...
     * @return std::unique_ptr<FileStorageInterface> Умный указатель на созданный адаптер
     * @throw std::invalid_argument При неподдерживаемом типе хранилища
     * 
     * @note Использует smart pointers для автоматического управления памятью[27][28]
     */
    std::unique_ptr<FileStorageInterface> createAdapter(const SourceConfig& config);
