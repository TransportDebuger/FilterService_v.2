    ...
    , source_metrics_(std::move(source_metrics))
{
    ...
    try {
        ...
        if (logger_) logger_->Info("Worker created for source: " + config_.name + " (type: " + config_.type + "), " + workerTag_);
    } catch (const std::exception &e) {
        if (logger_) logger_->Error("Failed to create worker: " + std::string(e.what()) + ", " + workerTag_);
        throw;
    }
}
...
void Worker::start() {
    ...
    try {
        ...
        if (logger_) logger_->Info("Worker started monitoring: " + config_.path + ", " + workerTag_);
    } catch (const std::exception &e) {
        running_ = false;
        if (logger_) logger_->Error("Failed to start worker: " + std::string(e.what()) + ", " + workerTag_);
        throw;
    }
}
...
void Worker::stop() {
    {
        ...
        running_ = false;
        paused_ = false;
    } // КРИТИЧНО: Освобождаем мьютекс перед join, чтобы Worker::run() мог завершиться
    ...
    try {
        while (running_) {
            ...
            if (std::chrono::steady_clock::now() - start_time_ > std::chrono::minutes(1)) {
                if (logger_) logger_->Debug("Worker stats - Avg duration: " + std::to_string(avg_duration_.load()) + "s, " + workerTag_);
            }
        }
    } catch (const std::exception &e) {
        if (logger_) logger_->Error("Worker crashed: " + std::string(e.what()) + ", " + workerTag_);
        running_ = false;
    }
}
...
void Worker::processFile(const std::string &filePath) {
    ...
    try {
        if (logger_) logger_->Debug("Processing file: " + filePath + ", " + workerTag_);
        ...
        if (config_.filtering_enabled) {
            // ИСПРАВЛЕНИЕ: Передаем инжектированный filter_list_manager_ в XMLProcessor
        ...
        if (logger_) logger_->Info("Successfully processed file: " + filePath + " (hash: " + fileHash.substr(0, 8) + "...) in " + std::to_string(duration) + "s, " + workerTag_);
    } catch (const std::exception &e) {
        if (source_metrics_.files_failed) source_metrics_.files_failed->Increment(1.0);
        if (global_metrics_.files_failed) global_metrics_.files_failed->Increment(1.0);
        if (source_metrics_.files_failed_parse) source_metrics_.files_failed_parse->Increment(1.0);
    ...
    for (const auto &path : paths) {
        if (!path.empty() && !fs::exists(path)) {
            try {
                ...
                if (logger_) logger_->Info("Created directory: " + path + ", " + workerTag_);
...
void Worker::moveToProcessed(const std::string &filePath, const std::string &processedPath) {
    try {
        ...
        if (logger_) logger_->Debug("Moved file from " + filePath + " to " + processedPath + ", " + workerTag_);
    } catch (const fs::filesystem_error &e) {
...
void Worker::handleFileError(const std::string &filePath, const std::string &error) {
    try {
        if (!config_.bad_dir.empty()) {
            ...
            if (logger_) logger_->Warning("Moved failed file to bad directory: " + badPath + ", " + workerTag_);
        }
        if (logger_) logger_->Error("Failed to process file " + filePath + ": " + error + ", " + workerTag_);
    } catch (const std::exception &e) {
        if (logger_) logger_->Error("Failed to handle file error: " + std::string(e.what()) + ", " + workerTag_);
    }
}
...
void Worker::restartMonitoring() {
    std::lock_guard<std::mutex> lock(state_mutex_);
    if (!adapter_) {
        if (logger_) logger_->Warning("Worker " + config_.name + " has no adapter to restart, " + workerTag_);
        return;
    }
    try {
        if (running_) {
            adapter_->stopMonitoring();
            if (logger_) logger_->Debug("Worker " + config_.name + " monitoring stopped, " + workerTag_);
            adapter_->startMonitoring();
            if (logger_) logger_->Info("Worker " + config_.name + " monitoring restarted, " + workerTag_);
        } else {
            if (logger_) logger_->Warning("Worker " + config_.name + " is not running, cannot restart monitoring, " + workerTag_);
        }
    } catch (const std::exception &e) {
        if (logger_) logger_->Error("Failed to restart monitoring for worker " + config_.name + ": " + std::string(e.what()) + ", " + workerTag_);
        throw;
    }
}
