...
SmbFileAdapter::SmbFileAdapter(const SourceConfig& config) 
    : config_(config) {
    ...
    if (it != config_.params.end()) domain_ = it->second;
    else domain_ = "WORKGROUP"; // Значение по умолчанию
    
    stc::CompositeLogger::instance().info(
        "SmbFileAdapter created for: " + smbUrl_
    );
}
...
std::vector<std::string> SmbFileAdapter::listFiles(const std::string& path) {
    ...
    try {
        std::string searchPath = path.empty() ? mountPoint_ : 
                                (fs::path(mountPoint_) / path).string();
        ...
        throw std::runtime_error("Failed to list SMB files: " + std::string(e.what()));
    }
...
void SmbFileAdapter::downloadFile(const std::string& remotePath, 
    ...
    try {
        ...
        stc::CompositeLogger::instance().info(
            "SMB file downloaded from " + sourcePath + " to " + localPath
        );
        
    } catch (const fs::filesystem_error& e) {
        stc::CompositeLogger::instance().error(
            "SMB download failed: " + std::string(e.what())
        );
        throw std::ios_base::failure("SMB file download failed: " + std::string(e.what()));
    }
}
...
void SmbFileAdapter::upload(const std::string& localPath,
    ...
    try {
        ...
        stc::CompositeLogger::instance().info(
            "File uploaded from " + localPath + " to SMB: " + targetPath
        );
        
    } catch (const fs::filesystem_error& e) {
        stc::CompositeLogger::instance().error(
            "SMB upload failed: " + std::string(e.what())
        );
        throw std::runtime_error("SMB file upload failed: " + std::string(e.what()));
    }
}
...
void SmbFileAdapter::connect() {
    ...
    try {
        ...
        stc::CompositeLogger::instance().info(
            "Connected to SMB share: " + smbUrl_ + " at " + mountPoint_
        );
        
    } catch (const std::exception& e) {
        stc::CompositeLogger::instance().error(
            "SMB connection failed: " + std::string(e.what())
        );
        throw std::runtime_error("Failed to connect to SMB: " + std::string(e.what()));
    }
}
...
void SmbFileAdapter::startMonitoring() {
    ...
    try {
        ...
        stc::CompositeLogger::instance().info(
            "Started SMB monitoring: " + mountPoint_
        );
        
    } catch (const std::exception& e) {
        stc::CompositeLogger::instance().error(
            "Failed to start SMB monitoring: " + std::string(e.what())
        );
        throw std::runtime_error("SMB monitoring start failed: " + std::string(e.what()));
    }
}
...
void SmbFileAdapter::mountSmbResource() {
    ...
    if (!fs::exists(mountPoint_) || !fs::is_directory(mountPoint_)) {
        throw std::runtime_error("SMB mount point validation failed: " + mountPoint_);
    }
}
...
void SmbFileAdapter::unmountSmbResource() {
    ...
    try {
        ...
        if (result == 0) {
            stc::CompositeLogger::instance().info("SMB resource unmounted successfully");
        } else {
            stc::CompositeLogger::instance().warning(
        ...
        stc::CompositeLogger::instance().error(
            "Error during SMB unmount: " + std::string(e.what())
        );
    }
}
...
std::string SmbFileAdapter::createMountPoint() {
    std::string tempDir = "/tmp/smb_mount_" + std::to_string(getpid()) + "_" + 
...
void SmbFileAdapter::handleFileEvent(FileWatcher::Event event, 
    ...
    try {
        if (event == FileWatcher::Event::Created) {
            ...
            if (matchesFileMask(filename)) {
                stc::CompositeLogger::instance().debug(
                    "New SMB file detected: " + filePath
                );
        ...
        stc::CompositeLogger::instance().error(
            "Error handling SMB file event: " + std::string(e.what())
        );
    }
}
...
bool SmbFileAdapter::matchesFileMask(const std::string& filename) const {
    try {
        ...
        stc::CompositeLogger::instance().warning(
            "Invalid SMB file mask regex: " + config_.file_mask
        );
        return true;
    }
}
