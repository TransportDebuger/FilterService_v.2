...
@brief No-Op (заглушки) реализации интерфейсов метрик и реестра для полного отключения сбора.
*/
#pragma once
...
@brief Пустая реализация реестра метрик. Возвращает No-Op дескрипторы без аллокаций на горячем пути.
*/
class NoOpMetricsRegistry final : public IMetricsRegistry {
public:
    /**
    @brief Конструирует реестр и инициализирует переиспользуемые No-Op дескрипторы.
    */
    NoOpMetricsRegistry()
...
private:
    /// @private Переиспользуемый дескриптор пустого счетчика (избегает аллокаций при регистрации).
